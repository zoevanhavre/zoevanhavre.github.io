...
```{r, echo=FALSE, warning=FALSE, fig.width = 5 , fig.height = 5, cache=TRUE}
	table(BigY$Type)
```

------

```{r, echo=FALSE, warning=FALSE, fig.width = 5 , fig.height = 5, cache=TRUE, warning=FALSE, message=FALSE}
require(ggplot2, quietly=TRUE)
g.hist.SURV<- ggplot(melt(BigY[,c(4:35,37)]), aes(value, group=variable)) 
g.hist.SURV<- g.hist.SURV+ geom_histogram(position="dodge")+ xlab("SURV")+ ylab("Frequency")+ facet_wrap(~variable,ncol=5 )+theme_bw()
g.hist.SURV<- g.hist.SURV+geom_vline(xintercept = 1.5, col=grey(.5), linetype='dotted')
g.hist.SURV
```
